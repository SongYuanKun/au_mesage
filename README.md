...
简要描述：这是一个用于采集、存储与查询黄金/白银价格的现代化Web应用。项目采用前后端分离架构，包含Playwright自动化采集、Flask REST API、以及现代化UI界面的完整解决方案。

主要目标：提供美观易用的贵金属价格监控界面，自动化数据采集，并为用户提供实时价格查询、历史趋势分析和智能价格计算功能。

## 📋 项目概览

本项目是一个现代化的金属价格监控平台，具有以下核心功能：

- **🤖 自动采集**：通过 Playwright 无头浏览器自动采集目标网站的黄金、白银等贵金属价格
- **🗄️ 数据持久化**：采用 MySQL 数据库存储价格历史，支持高效批量插入和查询优化
- **🚀 REST API**：提供完整的 Flask API，支持查询最新价格、历史数据、价格计算等
- **🎨 现代化UI**：采用现代化设计系统，包含渐变色彩、动画效果和响应式布局
- **📊 数据可视化**：内置 ECharts 图表，实时展示价格趋势和统计分析
- **🐳 容器化部署**：提供优化的 Dockerfile 和 docker-compose 配置，一键启动服务
- **📱 移动适配**：完整的响应式设计，支持移动端和桌面端访问

### 🌟 核心特性

✅ **现代化UI设计**：采用CSS变量系统、渐变背景、微动画和响应式布局  
✅ **智能价格计算器**：实时计算价格差异，支持黄金白银多种类型  
✅ **后台自动采集**：线程化运行，无阻塞API服务  
✅ **高效数据缓冲**：30秒内存缓冲 + 周期性批量写入优化  
✅ **多数据类型支持**：黄金、白银，易于扩展其他贵金属  
✅ **JSON序列化优化**：Decimal、datetime自动转换  
✅ **完整错误处理**：异常捕获和日志记录系统  
✅ **Docker容器化**：多阶段构建优化，包含浏览器依赖  
✅ **移动端适配**：完整的响应式设计和触摸优化  
✅ **实时数据推送**：SSE价格到达通知和自动刷新机制  
...
│   - 🎨 现代化设计系统：CSS变量、渐变、动画         │
│   - 📱 响应式布局：移动端和桌面端完美适配           │
│   - 📊 ECharts图表：实时价格趋势和统计分析          │
│   - 🧮 智能计算器：自动计算价格差异和收益           │
│   - 🔔 实时通知：SSE价格到达推送系统               │
│   - 🏠 index.html: 主页（实时价格、智能计算器）     │
│   - 📈 history.html: 趋势页面（7天趋势、30分钟数据）│
└─────────────────────────────────────────────────────┘
```
...
│
├── templates/                    # 前端模板目录
│   ├── index.html                # 主页 HTML（现代化UI、实时价格、智能计算器）
│   ├── history.html              # 趋势页面 HTML（7天趋势图、30分钟实时数据、统计卡片）
│   ├── today.html                # 当天实时数据表格页面
│   ├── _nav.html                 # 现代化导航栏模板（响应式、动画效果）
│   └── _styles.html              # 样式模板（已集成到main.css）
│
├── static/                       # 静态资源目录
│   └── css/
│       └── main.css              # 现代化样式文件（CSS变量系统、动画、响应式）
```
...
#### 主页功能
- **📊 实时价格显示**：动态更新的黄金/白银价格，带有渐变背景和脉冲动画
- **🧮 智能计算器**：自动计算价格差异和收益，支持实时输入
- **🔔 价格推送**：SSE实时价格到达通知系统
- **📱 响应式设计**：完美适配移动端和桌面端

#### 趋势页面功能
- **📈 30分钟实时图表**：动态更新的价格趋势图
- **📊 7天统计分析**：平均价、最高价、最低价统计卡片
- **🔄 自动刷新**：30秒自动更新数据
- **📱 移动优化**：触摸友好的交互设计

#### 设计特色
- **🎨 现代化设计系统**：CSS变量、渐变色彩、微动画
- **✨ 丰富的交互效果**：悬停动画、过渡效果、加载状态
...
本项目采用现代化的设计系统，提供一致性和可维护性：

#### 🎯 核心设计原则
- **一致性**：统一的颜色、字体、间距和交互模式
- **可访问性**：良好的对比度、键盘导航、屏幕阅读器支持
- **响应式**：移动优先的设计方法，适配所有设备
- **性能优化**：CSS动画硬件加速，优化加载性能
...
:root {
  ...
  --primary-dark: #5a67d8;       /* 主色调深色 */
  --secondary-color: #48bb78;    /* 辅助色 */
  --success-color: #48bb78;      /* 成功色 */
  --danger-color: #f56565;       /* 危险色 */
  --warning-color: #ed8936;      /* 警告色 */
  --info-color: #4299e1;         /* 信息色 */
}
```
...
### ✅ 已完成功能
- [x] **现代化UI设计系统**：CSS变量、渐变色彩、微动画
- [x] **响应式布局**：移动端和桌面端完美适配
- [x] **智能价格计算器**：实时计算价格差异和收益
- [x] **实时数据推送**：SSE价格到达通知系统
- [x] **统计卡片组件**：7天数据可视化展示
- [x] **现代化导航栏**：响应式汉堡菜单和动画效果
- [x] **性能优化**：CSS硬件加速和加载优化

### 🚧 进行中功能
- [ ] **数据导出功能**：支持Excel、CSV格式导出
- [ ] **高级图表功能**：更多图表类型和交互功能

### 📋 计划功能
- [ ] 支持更多数据类型（铂金、钯金等）
- [ ] 添加用户认证系统和权限管理
- [ ] 实现 WebSocket 实时推送替代 SSE
- [ ] 性能监控面板和系统健康检查
- [ ] 多语言国际化支持
- [ ] 数据备份和恢复功能
- [ ] API 限流和安全增强
