...
一个用于采集、存储与查询黄金/白银价格的轻量服务。基于 Playwright 无头浏览器自动采集价格数据，通过 Flask REST API 对外提供查询与计算接口，数据持久化到 MySQL。
...
- **Python 3.8+**
- **MySQL 5.7+** 或 **8.0+**
- **Docker** 和 **Docker Compose**（容器化部署时，可选）
